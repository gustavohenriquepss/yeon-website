...
const contactSchema = z.object({
  ...
  phone: z.string().trim().min(1, {
    ...
    message: "Telefone deve ter menos de 20 caracteres"
  }),
            ...
            <div className="bg-card/50 backdrop-blur-sm border border-white/10 rounded-lg p-8 md:p-12">
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                <div className="space-y-2">
                  ...
                  <Input id="name" {...register('name')} placeholder="Seu nome completo" className="bg-background/50 border-white/10" />
                  {errors.name && <p className="text-sm text-red-500">{errors.name.message}</p>}
                </div>

                <div className="space-y-2">
                  <Label htmlFor="email" className="text-white">Head of Product</Label>
                  <Input id="email" type="email" {...register('email')} placeholder="seu@email.com" className="bg-background/50 border-white/10" />
                ...
                <div className="space-y-2">
                  ...
                  <Input id="phone" type="tel" {...register('phone')} placeholder="(00) 00000-0000" className="bg-background/50 border-white/10" />
                ...
                <div className="space-y-2">
                  ...
                  <Textarea id="message" {...register('message')} placeholder="Conte-nos como podemos ajudar..." rows={6} className="bg-background/50 border-white/10 resize-none" />
                  {errors.message && <p className="text-sm text-red-500">{errors.message.message}</p>}
                </div>

                <Button type="submit" size="lg" disabled={isSubmitting} className="w-full bg-yeon-purple hover:bg-yeon-dark-purple text-white font-medium">
